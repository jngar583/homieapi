...
app.get("/extract", async (req, res) => {
  ...
  try {
    ...
    // Function to extract real Adsfly URL
    async function extractRealUrl(adsLink) {
      ...
      adPage.on("response", async response => {
        if (response.url().includes("/get-real-url") && response.request().method() === "POST") {
          try {
            ...
            console.log("Error parsing JSON for:", adsLink);
          }
        }
      });
      ...
      try {
        ...
        await adPage.waitForSelector("button.bg-red-600", { timeout: 15000 });
        await adPage.click("button.bg-red-600");
